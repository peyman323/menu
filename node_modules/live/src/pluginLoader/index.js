//region Imports
const {debug} = require('live-log')('live:plugin-loader')
  ...
  registerLiveModuleServices({module}) {
    ...
    const consumers = (obj) => {
      for (const keyPath in obj) {
        ...
        for (const version in versionToFnName) {
          ...
          let consumerCallback = module[fnName].bind(module)
          if (!consumerCallback) {
          ...
          this.serviceHub.consume(keyPath, version, consumerCallback)
        }
      }
    }
    ...
    const providers = (obj) => {
      for (const keyPath in obj) {
        ...
        for (const version in versionToFnName) {
          const fnName = versionToFnName[version]
          let serviceGetter = module[fnName].bind(module)
          if (!serviceGetter) {
          ...
          this.serviceHub.provide(keyPath, version, service)
        }
      }
    }
